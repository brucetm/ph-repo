...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<?php if ( ! bp_is_user() && ! bp_is_group_single() && ! bp_is_group_create()
            ...
            if ( function_exists( 'bp_is_register_page' ) && !is_user_logged_in() && bp_is_register_page()) { ?>
            	<p class="sigup-logo-subhdg">Master Of Products</p>
            	<p class="ph_signup_txt">Playhacker is free skills tutorials, creative DIY projects, interesting ideas, quality tools and materials, and a builder community!<p>
                <?php 
            } ?>      
		<header class="entry-header">
			<?php if ( function_exists( 'bp_is_register_page' ) && !is_user_logged_in() && bp_is_register_page()) { ?>
