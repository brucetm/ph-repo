...
/**Coming soon**/
$coming_soon='';
$coming_soon_course = get_field( "future_project" ); 
            ...
            <?php if ( has_post_thumbnail() ) {
				...
				if ( is_user_logged_in() && isset( $has_access ) && $has_access ) {
					...
					if ( ( $course_pricing['type'] === 'open' && $progress['percentage'] === 0 ) || ( $course_pricing['type'] !== 'open' && $has_access && $progress['percentage'] === 0 ) ) {
						...
						__( 'future build! ', 'buddyboss-theme' ) .
							'</div>';
                        }else{
             ...
             <?php if($coming_soon_course){ 
                echo '<div class="future-proj">';
                } else { 	 
